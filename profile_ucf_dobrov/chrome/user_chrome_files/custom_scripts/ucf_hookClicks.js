...
◧ лев.+ Alt  Оставить 1 текущую`,"identity-box": //отладка UCF ☑: команды|действия в консоль

`Правый клик	 Копировать адрес в буфер
◉, Клик + Alt	 Настройки прокси {︰\n
◧ лево + Shift	 Медиа на странице}
Ø колёсико ±	 Яркость страниц `,"favdirs-button": //боковая панель открыта: отладка Вкл
...
Alt + x		запустить скрипт User.js`,"titlebar-button.titlebar-close": //кроме Windows
...
Mouse = { // Meta*64 Ctrl*32 Шифт*16 Alt*8 (Wh ? 2 : But*128) long*1
	...
	[Bt[6]]: { mousedownTarget: true,
		...
		8(btn){ switchTab('chrome://browser/content/places/places.xhtml')}, //+ Alt
		256(){0}, // lock ПМ
...
window.glob = { //all [ChromeOnly]-scripts
	...
	dirsvcget(cat = "DfltDwnld",sub,open,dir) { // DfltDwnld GreD UChrm Home
		try {dir = Services.dirsvc.get(cat,Ci.nsIFile);} catch {dir = Services.prefs.getComplexValue("browser.download.dir",Ci.nsIFile)}
		sub && dir.append(sub);
		if (open) dir.exists() && dir.launch()
		else return dir; //объект|открыть папку
	},
